<template>
	<div id="app">
		<svg style="position: absolute; width: 0; height: 0;" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
			<defs>
				<symbol id="icon-close" viewBox="0 0 32 32">
					<title>close</title>
					<path d="M17.387 16l8.907-8.907c0.373-0.373 0.373-1.013 0-1.44-0.373-0.373-1.013-0.373-1.387 0l-8.907 8.907-8.907-8.853c-0.373-0.373-1.013-0.373-1.44 0-0.373 0.373-0.373 1.013 0 1.387l8.907 8.907-8.907 8.907c-0.373 0.373-0.373 1.013 0 1.44 0.373 0.373 1.013 0.373 1.387 0l8.907-8.907 8.907 8.907c0.373 0.373 1.013 0.373 1.387 0 0 0 0 0 0 0 0.373-0.373 0.373-1.013 0-1.44l-8.853-8.907z"></path>
				</symbol>
				<symbol id="icon-arrow" viewBox="0 0 32 32">
					<title>arrow</title>
					<path d="M21.653 15.093l-10.027-9.28c-0.427-0.373-1.067-0.32-1.387 0.053-0.373 0.427-0.32 1.013 0.053 1.387l9.227 8.587-9.227 8.96c-0.373 0.373-0.427 1.013 0 1.44 0.373 0.373 1.013 0.427 1.44 0l10.027-9.707c0.32-0.373 0.32-1.013-0.107-1.44 0.053 0 0 0 0 0z"></path>
				</symbol>
				<symbol id="icon-expansion" viewBox="0 0 32 32">
					<title>expansion</title>
					<path d="M17.013 25.973l9.707-10.4c0.373-0.427 0.373-1.067-0.053-1.44s-1.067-0.373-1.44 0.053l-8.907 9.6-9.333-9.653c-0.373-0.427-1.067-0.427-1.44-0.053-0.427 0.373-0.427 1.067-0.053 1.44l10.080 10.4c0.213 0.213 0.48 0.32 0.747 0.32 0.213 0.053 0.48-0.053 0.693-0.267z"></path>
					<path d="M17.013 17.867l9.707-10.4c0.373-0.427 0.373-1.067-0.053-1.44s-1.067-0.373-1.44 0.053l-8.907 9.653-9.333-9.653c-0.373-0.427-1.067-0.427-1.44-0.053-0.427 0.373-0.427 1.067-0.053 1.44l10.080 10.4c0.213 0.213 0.48 0.32 0.747 0.32s0.48-0.107 0.693-0.32z"></path>
				</symbol>
				<symbol id="icon-search" viewBox="0 0 32 32">
					<title>search</title>
					<path d="M6.987 14.507c0-4.427 3.573-8 8-8s8 3.573 8 8-3.573 8-8 8c-4.427 0-8-3.573-8-8zM26.72 25.813l-4.48-4.48c1.76-1.867 2.72-4.32 2.72-6.88 0-5.547-4.48-10.027-10.027-10.027s-10.027 4.48-10.027 10.027 4.48 10.027 10.027 10.027c2.080 0 4.053-0.64 5.76-1.813l4.587 4.587c0.373 0.373 1.013 0.373 1.44 0 0.427-0.427 0.427-1.013 0-1.44z"></path>
				</symbol>
				<symbol id="icon-selected" viewBox="0 0 32 32">
					<title>selected</title>
					<path d="M25.707 8.32c-0.32-0.373-0.907-0.373-1.227-0.053-0.053 0.053-0.053 0.053-0.107 0.107l-11.040 13.067-5.707-6.667c-0.32-0.373-0.853-0.427-1.227-0.107-0.053 0.053-0.053 0.053-0.107 0.107-0.373 0.427-0.373 1.067-0.053 1.493l6.4 7.467c0.16 0.213 0.427 0.32 0.693 0.32v0c0.267 0 0.533-0.107 0.693-0.32l11.733-13.867c0.32-0.48 0.32-1.12-0.053-1.547z"></path>
				</symbol>
			</defs>
		</svg>
		<keep-alive>
			<router-view v-if="$route.meta.keepAlive" class="view"></router-view>
		</keep-alive>
		<router-view v-if="!$route.meta.keepAlive" class="view"></router-view>
		<div class="loading" v-if="isLoading">
			<svg class="icon icon-spinner3">
				<symbol id="icon-spinner3" viewBox="0 0 32 32">
					<title>spinner3</title>
					<path d="M16 9.472c-1.030 0-1.865-0.835-1.865-1.865v-5.596c0-1.030 0.835-1.865 1.865-1.865s1.865 0.835 1.865 1.865v5.596c0 1.030-0.835 1.865-1.865 1.865z"></path>
					<path d="M16 31.155c-0.644 0-1.166-0.522-1.166-1.166v-5.596c0-0.644 0.522-1.166 1.166-1.166s1.166 0.522 1.166 1.166v5.596c0 0.644-0.522 1.166-1.166 1.166z"></path>
					<path d="M11.805 10.48c-0.604 0-1.192-0.314-1.516-0.875l-2.798-4.846c-0.483-0.836-0.196-1.906 0.64-2.389s1.906-0.196 2.389 0.64l2.798 4.846c0.483 0.836 0.196 1.906-0.64 2.389-0.275 0.159-0.576 0.235-0.873 0.235z"></path>
					<path d="M22.995 29.164c-0.363 0-0.715-0.188-0.91-0.525l-2.798-4.846c-0.29-0.502-0.118-1.143 0.384-1.433s1.143-0.118 1.433 0.384l2.798 4.846c0.29 0.502 0.118 1.143-0.384 1.433-0.165 0.095-0.346 0.141-0.524 0.141z"></path>
					<path d="M8.729 13.436c-0.277 0-0.557-0.070-0.814-0.219l-4.846-2.798c-0.781-0.451-1.048-1.449-0.597-2.229s1.449-1.048 2.229-0.597l4.846 2.798c0.781 0.451 1.048 1.449 0.597 2.229-0.302 0.524-0.851 0.816-1.415 0.816z"></path>
					<path d="M28.114 23.927c-0.158 0-0.319-0.040-0.465-0.125l-4.846-2.798c-0.446-0.258-0.599-0.828-0.341-1.274s0.828-0.599 1.274-0.341l4.846 2.798c0.446 0.258 0.599 0.828 0.341 1.274-0.173 0.299-0.486 0.466-0.809 0.466z"></path>
					<path d="M7.607 17.515h-5.596c-0.837 0-1.516-0.678-1.516-1.515s0.678-1.515 1.516-1.515h5.596c0.837 0 1.516 0.678 1.516 1.515s-0.678 1.515-1.516 1.515z"></path>
					<path d="M29.989 16.933c-0 0 0 0 0 0h-5.596c-0.515-0-0.933-0.418-0.933-0.933s0.418-0.933 0.933-0.933c0 0 0 0 0 0h5.596c0.515 0 0.933 0.418 0.933 0.933s-0.418 0.933-0.933 0.933z"></path>
					<path d="M3.886 24.394c-0.483 0-0.954-0.251-1.213-0.7-0.386-0.669-0.157-1.525 0.512-1.911l4.846-2.798c0.669-0.387 1.525-0.157 1.911 0.512s0.157 1.525-0.512 1.911l-4.846 2.798c-0.22 0.127-0.461 0.188-0.698 0.188z"></path>
					<path d="M23.27 12.736c-0.322 0-0.636-0.167-0.809-0.466-0.258-0.446-0.105-1.016 0.341-1.274l4.846-2.798c0.446-0.257 1.016-0.105 1.274 0.341s0.105 1.016-0.341 1.274l-4.846 2.798c-0.147 0.085-0.307 0.125-0.465 0.125z"></path>
					<path d="M9.004 29.397c-0.218 0-0.438-0.055-0.64-0.172-0.613-0.354-0.823-1.138-0.469-1.752l2.798-4.846c0.354-0.613 1.138-0.823 1.752-0.469s0.823 1.138 0.469 1.752l-2.798 4.846c-0.237 0.411-0.668 0.641-1.112 0.641z"></path>
					<path d="M20.196 9.664c-0.158 0-0.319-0.040-0.465-0.125-0.446-0.258-0.599-0.828-0.341-1.274l2.798-4.846c0.258-0.446 0.828-0.599 1.274-0.341s0.599 0.828 0.341 1.274l-2.798 4.846c-0.173 0.299-0.486 0.466-0.809 0.466z"></path>
				</symbol>
				<use xlink:href="#icon-spinner3"></use>
			</svg>
		</div>
	</div>
</template>
<script>
import { mapState } from "vuex";
export default {
	name: "App",
	data() {
		return {};
	},
	computed: {
		...mapState(["isLoading"])
	},
	methods: {}
};
</script>

<style lang="scss">
button {
	font-size: 32px;
	display: block;
	padding-left: 14px;
	padding-right: 14px;
	box-sizing: border-box;
	text-align: center;
	text-decoration: none;
	line-height: 2.55555556;
	border-radius: 10px;
	-webkit-tap-highlight-color: transparent;
	overflow: hidden;
	color: #000000;
	background-color: #f8f8f8;
}
button[size="mini"] {
	display: inline-block;
	line-height: 2.3;
	font-size: 26px;
	padding: 0 1.34em;
}

button[type="primary"] {
	color: #ffffff;
	background-color: #1aad19;
}
button[type="default"] {
	color: #000000;
	background-color: #f8f8f8;
	border: 1px solid #ddd;
}
button[type="warn"] {
	color: #ffffff;
	background-color: #e64340;
}

.loading {
	width: 100px;
	height: 100px;
	display: inline-block;
	stroke-width: 0;
	stroke: currentColor;
	fill: currentColor;
	z-index: 10;
	position: fixed;
	left: 50%;
	top: 50%;
	transform: translate3d(-50%, -50%, 0);
}
@keyframes spinner {
	0% {
		transform: rotate(360deg);
	}
	100% {
		transform: rotate(0deg);
	}
}
.loading svg {
	transform-origin: center;
	animation: spinner 0.8s infinite linear;
	width: 100px;
	height: 100px;
}

* {
	margin: 0;
	padding: 0;
	box-sizing: border-box;
}

body {
	margin: 0;
	font-size: 24px;
	background-color: #f7f7f7;
}
a {
	color: #1e1e1e;
	text-decoration: none;
}
input:focus,
textarea:focus {
	outline: 0;
	border: 0;
	-webkit-tap-highlight-color: rgba(0, 0, 0, 0);
	-webkit-appearance: none;
}
input,
button,
select,
textarea {
	outline: 0;
	border: 0;
	margin: 0;
	padding: 0;
}

input:not([type="checkbox"]) {
	-webkit-tap-highlight-color: rgba(0, 0, 0, 0);
	-webkit-appearance: none;
}

/* *:not(input, textarea) {
    -webkit-touch-callout: none;
    -webkit-user-select: none;
} */

li,
ol,
ul {
	list-style: none;
}

div[class*="-page view"] {
	color: #1e1e1e;
	position: absolute;
	top: 0;
	bottom: 0;
	left: 0;
	right: 0;
	display: flex;
	flex-direction: column;
	background-color: #fafaf8;
}

.overflow-container {
	flex: 1;
	position: relative;
	overflow-y: auto;
	overflow-x: hidden;
	-webkit-overflow-scrolling: touch;
	z-index: 1;
}
</style>
