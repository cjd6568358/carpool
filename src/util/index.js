...
const getCurrCityByIP = async () => {
    let result = ''
    let { status, content } = await jsonp('https://api.map.baidu.com/location/ip?ak=qkwXKuCY5GsctKcaopqTGbnl');
    if (status === 0) {
        result = content.address_detail.province
    }
    return result
}
